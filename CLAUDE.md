...
- Keep all testable logic in the `WaterFinderCore` Swift package (see HANDOFF.md). It must import only Foundation, so it builds and tests on Linux with `swift build` / `swift test`.
...
- The container network may block overpass-api.de and openstreetmap.org. Do not depend on live OSM calls in tests. Use fixture JSON files in `WaterFinderCore/Tests/Fixtures/`.
...
- The classification rules exist in two places: `app/.../WaterSpot.swift` and `tools/explorer/index.html`. When you move them into `WaterFinderCore`, keep the explorer in sync or note the difference.
...
- Add or update tests in `WaterFinderCore` for every logic change.
